...
grade_trade_wrapper <- function(assets_df, mvt, team1_players, team2_players){
  ...
  ov_statement <- str_c("Trade Total Future Value Favors: ", ov_favor, " (", round(abs(overall_value), digits = 2), ")")
  ...
  iv_statement <- str_c("Trade Current Season Value Favors: ", iv_favor, " (", round(abs(immediate_value), digits = 2), ")")
  
  list("team1" = janitor::adorn_totals(team1_assets_gained),
...
graph_trade <- function(team1_assets_received, team2_assets_received){
  bind_rows(
    slice_tail(team1_assets_received, n=1) %>% mutate(team = team1_assets_received$team[1]),
    slice_tail(team2_assets_received, n=1) %>% mutate(team = team2_assets_received$team[1])) %>%
    select(team, future_value, upcoming_year) %>%
    pivot_longer(cols = c(future_value, upcoming_year), names_to = "type", values_to = "value") %>%
    mutate(type = recode(type, future_value = "Future Value", upcoming_year = "Current Season")) %>%
    ggplot() +
    geom_bar(aes(x = type, y = value, fill = team), stat = "identity", position = position_dodge(width = 1), width = 1) +
    labs(x = "", y = "") +
