...
plot_future_value <- function(enter_name) {
  plot_future_value_df %>%
    ...
    geom_ribbon(
      aes(ymin = proj_tva_10, ymax = proj_tva_90),
      fill = "cadetblue4",
      alpha = .5
    ) +
    geom_ribbon(
      aes(ymin = proj_tva_2.5, ymax = proj_tva_97.5),
      fill = "cadetblue1",
      alpha = .5
    ) +
    labs(
      title = str_c(enter_name, " Projected Total Value Added"),
      x = "",
      y = ""
    ) +
    geom_point(
      data = season_value_added %>% filter(name == enter_name),
      aes(x = season, y = total_value_added)
    )
}

tabulate_realized_value <- function(
  va_data,
  enter_name,
  enter_season,
  shiny = FALSE
  ...
  if (shiny) {
    ...
    df %>%
      ...
      fmt_number(
        columns = c(`total value added`, `mean fantasy points`),
        decimals = 2
      ) %>%
...
# top contributors
grab_team_contributors <- function(
  enter_roster_id,
  enter_season,
  shiny = FALSE
) {
  ...
  if (shiny) {
    ...
    df %>%
      ...
      gt_theme_538(quiet = TRUE) %>%
      fmt_number(
        columns = c(total_value_added, fantasy_points),
        decimals = 2
      ) %>%
      cols_label(
        total_value_added = "Total Value Added",
...
# week by week
grab_team_contributors_weekly <- function(
  enter_roster_id,
  enter_season,
  enter_week,
  shiny = FALSE
  ...
  if (shiny) {
    ...
    df %>%
      ...
      gt_theme_538() %>%
      fmt_number(
        columns = c(value_added, sleeper_points, projection),
        decimals = 2
      ) %>%
      cols_label(
        value_added = "Value Added",
        sleeper_points = "Fantasy Points"
      ) %>%
      tab_header(
        title = str_c(
          team_name,
...
overall_grades <- function(value_avenues, enter_roster_id, shiny = FALSE) {
  ...
  df <- value_avenues %>%
    filter(roster_id == enter_roster_id) %>%
    relocate(
      c(avenue, total_realized_value, total_future_value),
      value_over_expected
    ) %>%
    select(-roster_id) %>%
    arrange(avenue) %>%
  ...
  if (shiny) {
    ...
    df %>%
      ...
      cols_label(
        total_realized_value = "Realized Value",
        total_future_value = "Future Value",
...
top_acquisitions <- function(
  ...
  if (shiny) {
    ...
    df %>%
      ...
      gt_theme_538(quiet = TRUE) %>%
      fmt_number(
        columns = c(realized_value, future_value, value_over_expected),
        decimals = 2
      ) %>%
      cols_label(
        realized_value = "Realized Value",
...
best_picks <- function(enter_draft, shiny) {
  ...
  if (shiny) {
    ...
    df %>%
      ...
      gt_theme_538() %>%
      fmt_number(
        columns = c(value_over_expected, realized_value, future_value),
        decimals = 2
      ) %>%
      cols_label(
...
weekly_results <- function(va_data, enter_name, enter_season) {
  va_data %>%
    ...
    pivot_longer(
      cols = c(sleeper_points, projection, value_added),
      names_to = "type",
      values_to = "value"
    ) %>%
    ggplot() +
...
inspect_individual_trade <- function(trade_id, shiny = FALSE) {
  title <- total_trade_value[[trade_id]] %>%
    select(team_name) %>%
    distinct() %>%
    map_chr(~ str_c(.x, collapse = ", ")) %>%
    str_c(
      total_trade_value[[trade_id]]$season[1],
      " W",
      total_trade_value[[trade_id]]$week[1],
      " Trade between ",
      .
    )
  ...
  df <- total_trade_value[[trade_id]] %>%
    ...
    group_by(team_name) %>%
    group_map(~ .x %>% adorn_totals("row"), .keep = TRUE) %>%
    bind_rows()
  ...
  if (shiny) {
    ...
    df %>%
      ...
      gt_theme_538(quiet = TRUE) %>%
      fmt_number(
        columns = c(future_value, realized_value, total_value),
        decimals = 2
      ) %>%
      cols_label(
        team_name = "Team",
...
inspect_individual_transaction <- function(transaction_id, shiny = FALSE) {
  title <- total_transaction_value[[transaction_id]]$team_name[1] %>%
    str_c(
      .,
      "'s ",
      total_transaction_value[[transaction_id]]$season[1],
      " W",
      total_transaction_value[[transaction_id]]$week[1],
      " Transaction"
    )
...
team_matchup_record <- function(
  ...
  df <- team_records_df %>%
    filter(
      str_detect(team, enter_team),
      round %in% enter_round,
      season %in% enter_seasons
    ) %>%
    group_by(opponent) %>%
    summarize(
      games = n(),
  ...
  player_total_value %>%
    ...
    mutate(
      ...
      age = lubridate::time_length(
        lubridate::interval(birth_date, today()),
        unit = "years"
      )
    ) %>%
    ungroup() %>%
...
comparable_players <- function(
  future_value_time,
  player_total_value,
  enter_name
) {
  player_position <- player_total_value %>%
    filter(name == enter_name) %>%
    pull(position)
...
graph_elo <- function(weekly_elo) {
  p <- weekly_elo %>%
    ...
    scale_x_continuous(
      breaks = weekly_elo$date_hide,
      labels = weekly_elo$date
    ) +
...
elo_win_probability <- function(team1_elo_df, team2_elo_df) {
  ...
  bind_rows(team1_elo_df, team2_elo_df) %>%
    ungroup() %>%
    mutate(
      win_probability = scales::percent(c(perc, 1 - perc), accuracy = .01)
    ) %>%
...
highest_team_total <- function(wins_df, enter_round, enter_season) {
  ...
  wins_df %>%
    arrange(desc(points)) %>%
    filter(type %in% enter_round, season %in% enter_season) %>%
    mutate(date = str_c(season, " Week ", week), index = row_number()) %>%
...
compute_total_points_player <- function(
  value_added,
  wins_df,
  enter_round,
  enter_season,
  ...
  value_added %>%
    left_join(
      wins_df %>% select(season, week, type, roster_id),
      by = join_by(season, week, roster_id)
    ) %>%
    filter(
      type.y %in% enter_round,
      season %in% enter_season,
      position %in% enter_position
    ) %>%
    group_by(name) %>%
...
compute_value_added_player <- function(
  value_added,
  wins_df,
  enter_round,
  enter_season,
...
grade_trade_wrapper <- function(assets_df, mvt, team1_players, team2_players) {
  ...
  team1_assets_gained <- team2_assets_lost %>%
    mutate(team = team1_assets_lost$display_name[1]) %>%
    select(team, name, position, future_value, upcoming_year)

  team2_assets_gained <- team1_assets_lost %>%
    mutate(team = team2_assets_lost$display_name[1]) %>%
    select(team, name, position, future_value, upcoming_year)

  roster_size_adj <- length(team1_players) -
    length(team2_players) -
    sum(str_count(team1_players, "Draft")) +
    sum(str_count(team2_players, "Draft"))

  if (roster_size_adj > 0) {
    team1_assets_gained <- team1_assets_gained %>%
      add_row(
        team = team1_assets_gained$team[1],
        name = "roster size adjustment",
        position = "gained",
        future_value = filter(
          mvt,
          season == max(mvt$season),
          type == "add"
        )$total_value_added[1] *
          roster_size_adj,
    ...
    team2_assets_gained <- team2_assets_gained %>%
      add_row(
        team = team2_assets_gained$team[1],
        name = "roster size adjustment",
        position = "lost",
        future_value = filter(
          mvt,
  ...
  overall_value <- sum(team1_assets_gained$future_value) -
    sum(team2_assets_gained$future_value)
  ov_favor <- if_else(
    overall_value > 0,
    team1_assets_gained$team[1],
    team2_assets_gained$team[1]
  )

  ov_statement <- str_c(
    "Trade Total Future Value Favors: ",
    ov_favor,
    " (",
    round(abs(overall_value), digits = 2),
    ")"
  )

  immediate_value <- sum(team1_assets_gained$upcoming_year) -
    sum(team2_assets_gained$upcoming_year)
  iv_favor <- if_else(
    immediate_value > 0,
    team1_assets_gained$team[1],
    team2_assets_gained$team[1]
  ...
  iv_statement <- str_c(
    "Trade Current Season Value Favors: ",
    iv_favor,
    " (",
    round(abs(immediate_value), digits = 2),
    ")"
  )

  list(
    "team1" = janitor::adorn_totals(team1_assets_gained),
    "team2" = janitor::adorn_totals(team2_assets_gained),
    "ov_statement" = ov_statement,
    "iv_statement" = iv_statement
  )
}

graph_trade <- function(team1_assets_received, team2_assets_received) {
  bind_rows(
    slice_tail(team1_assets_received, n = 1) %>%
      mutate(team = team1_assets_received$team[1]),
    slice_tail(team2_assets_received, n = 1) %>%
      mutate(team = team2_assets_received$team[1])
  ) %>%
    select(team, future_value, upcoming_year) %>%
    pivot_longer(
      cols = c(future_value, upcoming_year),
      names_to = "type",
      values_to = "value"
    ) %>%
    mutate(
      type = recode(
        type,
        future_value = "Future Value",
        upcoming_year = "Current Season"
      )
    ) %>%
    ggplot() +
    geom_bar(
      aes(x = type, y = value, fill = team),
      stat = "identity",
