...
rookie_drafts <- bind_rows(draft_picks, .id = "draft_id") %>%
  ...
  mutate(
    ...
    season = if_else(
      draft_id == 1,
      as.numeric(draft_id) + 2023,
      as.numeric(draft_id) + 2022
    )
  ) %>%
  ungroup() %>%
  ...
  left_join(
    player_total_value %>% select(-player_id, -position),
    by = join_by(name)
  ) %>%
  mutate(
...
tibble(.pred = quantiles_tv[20, ], pick_no = c(1:36)) %>%
  right_join(rookie_drafts, by = join_by(pick_no)) %>%
...
# plot residuals against pick_no, looks ok enough
tibble(.pred = quantiles_ny[20, ], pick_no = c(1:36)) %>%
  right_join(ny_data, by = join_by(pick_no)) %>%
...
tic() #note that X_tv[c(1:36)] can actually stay the same
samples_ny2 <- het_reg_mh_sampler(
  Y = Y_ny2,
  X = X_ny2,
  new_X = X_tv[c(1:36), ],
  iter = 50000,
...
# plot residuals against pick_no, looks ok enough
tibble(.pred = quantiles_ny2[20, ], pick_no = c(1:36)) %>%
  right_join(ny2_data, by = join_by(pick_no)) %>%
...
tic() #note that X_tv[c(1:36)] can actually stay the same
samples_ny3 <- het_reg_mh_sampler(
  Y = Y_ny2,
  X = X_ny2,
  new_X = X_tv[c(1:36), ],
  iter = 50000,
...
# plot residuals against pick_no, looks ok enough
tibble(.pred = quantiles_ny3[20, ], pick_no = c(1:36)) %>%
  right_join(ny3_data, by = join_by(pick_no)) %>%
...
quantiles_list <- list(
  "total value" = quantiles_tv,
  "first year" = quantiles_ny,
  "second year" = quantiles_ny2,
  "third_year" = quantiles_ny3
)

rookie_draft_values <- imap_dfr(
  1:4,
  ~ {
