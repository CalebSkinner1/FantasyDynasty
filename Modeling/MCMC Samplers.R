...
reg_gibbs_sampler <- function(
  ...
  for (i in 1:iter) {
    ...
    # draw sigma^2
    sigma_prop <- rinvgamma(
      1,
      (nu_0 + n * p) / 2,
      (nu_0 * sigma_prop + t((Y - X %*% beta_prop)) %*% (Y - X %*% beta_prop)) /
        2
    )
    ...
    if (i > burn_in & (i - burn_in - 1) %% thin == 0) {
      ...
      new_y[id, ] <- new_X %*%
        beta_prop +
        rnorm(nrow(new_X), 0, sigma_prop^(.5))
    }
  }
...
het_reg_mh_sampler <- function(
  ...
  for (i in 1:iter) {
    ...
    for (j in 1:p) {
      ...
      accept_prob <- exp(min(
        0,
        hetr_log_accept(gamma_new, gamma_prop, X, Y - X %*% beta_prop)
      ))
    ...
    if (i > burn_in & (i - burn_in - 1) %% thin == 0) {
      ...
      new_y[id, ] <- new_X %*%
        beta_prop +
        rnorm(nrow(new_X), 0, new_sigma_prop^(.5))
    }
  }
...
hlr_gibbs_sampler <- function(
  ...
  for (i in 1:iter) {
    ...
    for (j in 1:J) {
      ...
      l <- invSigma_prop %*%
        beta_prop +
        invsigma_j_prop[j] * (t(X_group[[j]]) %*% Y_group[[j]])

      # draw beta_j
      beta_j_prop[j, ] <- MASS::mvrnorm(1, invQ %*% l, invQ)

      # draw invsigma_j
      invsigma_j_prop[j] <- rgamma(
        1,
        shape = (nu_0 + n) / 2,
        rate = (nu_0 *
          zeta_prop +
      ...
      S_0 +
        Reduce(
          ...
          map(
            1:J,
            ~ {
              (beta_j_prop[.x, ] - beta_prop) %*%
                t(beta_j_prop[.x, ] - beta_prop)
            }
          )
        )
    )
    ...
    zeta_prop <- rgamma(
      1,
      shape = a_0 + J * nu_0 / 2,
      scale = b_0 + nu_0 / 2 * sum(invsigma_j_prop)
    )
    ...
    if (i > burn_in & (i - burn_in - 1) %% thin == 0) {
      ...
      y_prop <- map(
        ...
        ~ {
          X_group[[.x]] %*%
            beta_j_prop[.x, ] +
            rnorm(nrow(X_group[[.x]]), 0, invsigma_j_prop[.x]^(-.5))
        }
      )
